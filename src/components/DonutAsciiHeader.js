...
const DonutAsciiHeader = () => {
  ...
  useEffect(() => {
    ...
    const intervalId = setInterval(() => {
        const b = [], z = [];
        const canvasWidth = 60, canvasHeight = 30;
        A += 0.05; B += 0.03;
        ...
        for (let j = 0; j < 6.28; j += 0.07) {
            ...
            for (let i = 0; i < 6.28; i += 0.02) {
                ...
                const R2 = 2.5, K2 = 5;
                const h = ct + R2; const D = 1 / (sp * h * sA + st * cA + K2);
                ...
                if (y < canvasHeight && y >= 0 && x >= 0 && x < canvasWidth && D > z[o]) {
                    z[o] = D; b[o] = luminanceChars[N > 0 ? N : 0];
                }
            }
        }
        setFrame(b.join(""));
  ...
  // FIX: Testo più piccolo su mobile e più grande su desktop
  return <pre className="text-center my-2 text-xs md:text-lg leading-tight text-yellow-400">{frame}</pre>;
};

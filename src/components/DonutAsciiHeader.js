...
const DonutAsciiHeader = () => {
  ...
  useEffect(() => {
    let A = 0; // Angolo di rotazione sull'asse X
    let B = 0; // Angolo di rotazione sull'asse Z
    
    const intervalId = setInterval(() => {
        const b = []; // buffer dei caratteri ASCII
        const z = []; // z-buffer per la profondità
  ...
  // FIX: Rimosso il margine verticale my-2 per un layout più compatto
  return <pre className="text-center text-lg leading-tight text-yellow-400">{frame}</pre>;
};
